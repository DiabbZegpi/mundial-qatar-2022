...
for (i in seq_len(1e5)) {
  ...
  resultados_match$resultado <- replicate(10, sample(c('local', 'visita', 'empate'), 1))
...
ggsave(plot = p, filename = here('mundial.png'),
       width = 9, dpi = 500)

```

```{r}
saveRDS(resultados, file = here('resultados.rds'))
```
